...
! ... compute some quantities for a central event
	...
	targetfac=targ%M/amu/3.75914d+6/(targ%abundancy/100.)
...
! ... keep the human interested
     ...
     >		wtcontribute*luminosity/ntried
	    if (debug(7))write (6,*)'wtcontribute = ',wtcontribute,' luminosity = ',luminosity
...
! ... xptar is multiplied by cos(theta) to get the xptar offsets.
	...
	if (mc_smear) then
	  if (debug(7))write(6,*) 'orig%Ein = ',orig%Ein,' main%target%teff(1) = ',main%target%teff(1)
...
! ... multiple scattering
	...
	if (mc_smear) then
	  ...
	  if (debug(7))write(6,*) 'orig%p%p = ',orig%p%p,' main%target%teff(3) = ',main%target%teff(3)
...
! ... multiple scattering
	...
	if (mc_smear) then
	  if (debug(7))write(6,*) 'orig%e%p = ',orig%e%p,' main%target%teff(2) = ',main%target%teff(2)
