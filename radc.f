...
! ... obtain y region: from (Egamma_lo/Egamma_hi)**g to 1, generate flat y.
	if (debug(7))write(6,*)'3333 Egamma = ',Egamma
	ymin = power_lo/power_hi
	y = ymin + grnd()*(1.-ymin)
	x = y**(1./g(itail))
	Egamma = x*Egamma_hi
	if (debug(7))write(6,*)'y = ',y,' g = ',g,' itail  = ',itail,' g(itail) = ',g(itail)
	if (debug(7))write(6,*)'4444 Egamma = ',Egamma,' x = ',x,' Egamma_hi = ',Egamma_hi
...
! ... ntup.radarm=ntail, or 12 if tail 2 is forced.
	...
	success = .false.
	rad_weight = 1
	if (debug(7)) write(6,*)'Egamma_used(1) = ',Egamma_used(1)
	do i = 1,3
	  Egamma_used(i) = 0.0
	enddo
	if (debug(7)) write(6,*)'Egamma_used(1) = ',Egamma_used(1)
...
c	    ebeam_max = Mp*edge%e%E%max / (Mp-edge%e%E%max*(1.-vertex%ue%z))
	    ebeam_max = Mh*edge%e%E%max / (Mh-edge%e%E%max*(1.-vertex%ue%z))
	    if (ebeam_max.lt.0) ebeam_max=1.d10
c	    ebeam_min = Mp*edge%e%E%min / (Mp-edge%e%E%min*(1.-vertex%ue%z))
	    ebeam_min = Mh*edge%e%E%min / (Mh-edge%e%E%min*(1.-vertex%ue%z))
...
! ... adjust kinematics and call complete_ev.  If complete_ev fails, return.
	  if (debug(7)) write(6,*)'!!!vertex%Ein = ',vertex%Ein,' Egamma_used(1) = ',Egamma_used(1)
