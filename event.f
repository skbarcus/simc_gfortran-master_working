...
! Generate Electron Angles (all cases):
	...
	if (debug(7)) write(6,*)'$$$$$$$$$$$$$$$$$ vertex%e%xptar = ',vertex%e%xptar,
     > ' gen%e%xptar%min = ',gen%e%xptar%min,' gen%e%xptar%max = ',gen%e%xptar%max
...
! NOTE: this needs to be done again for the exclusive rho stuff (just on the hadron side).
	if (debug(7))write(6,*)'^^^^^^^^',spec%e%theta,spec%e%phi,
...
! Calculate scattered electron energy for hydrogen/deuterium (e,e'p)
	...
	if (doing_hyd_elast) then
	  ...
	  if (debug(7)) write(6,*)'vertex%Ein = ',vertex%Ein,' Mh = ',Mh,' vertex%ue%z = ',vertex%ue%z
